...
namespace ImageReadCS
{
    public unsafe partial class ImageIO
    {
                ...
                try
                {
                    for (int j = 0; j < H; j++)
                        for (int i = 0; i < W; i++)
                        {
                            ...
                            int g = pal[c * 4 + 1];
                            int r = pal[c * 4 + 2];
                            res[i, j] = new ColorPixel<float>() { b = b, g = g, r = r, a = 0.0f };
                        }
                }
                finally
                {
                ...
                try
                {
                    for (int j = 0; j < H; j++)
                        for (int i = 0; i < W; i++)
                        {
                            ...
                            byte g = pal[c * 4 + 1];
                            byte r = pal[c * 4 + 2];
                            res[i, j] = new ColorPixel<byte>() { b = b, g = g, r = r, a = 255 };
                        }
                }
                finally
                {
        ...
        public static void OpenInWindowViewer(string pathFile) {
            PowerShell ps = PowerShell.Create();
            ps.AddCommand(pathFile);
            ps.Invoke();
        }
